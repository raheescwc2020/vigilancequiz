...
const N8N_WEBHOOK_URL = 'https://raheesmohammed.app.n8n.cloud/webhook-test/917f300d-1806-4e9d-a54b-c247d0953c0a';
...
const Question = ({ question, onAnswer, onSkip, selectedAnswer }) => {
  // The handleOptionClick function now simply calls onAnswer with the new selection.
  // Because 'onAnswer' updates the state, and 'selectedAnswer' re-renders the component,
  // the user can click any option at any time to change their answer.
  ...
  return (
    <div className="w-full bg-white p-6 rounded-xl shadow-xl border border-gray-100 mb-6">
      ...
      <div className="mt-6 flex justify-end">
        {/* Only show the skip button if not already answered or skipped */}
        {selectedAnswer === undefined && (
          <button
            onClick={() => onSkip(question.id)}
            className="text-sm px-4 py-2 text-red-500 border border-red-300 rounded-lg hover:bg-red-50 transition-colors"
          >
            Skip Question
          </button>
        )}
...
const App = () => {
  ...
  const skippedQuestions = questions.filter(q => userAnswers[q.id] === undefined || userAnswers[q.id] === null);
  ...
  const sendQuizResultsToN8n = useCallback(async () => {
    ...
    const timeTaken = TOTAL_TIME_SECONDS - timeLeft;
  ...
  }, [employeeName, employeeEmail, timeLeft, userAnswers, calculateScore]);
  ...
  const handleAnswer = (questionId, answer) => {
    // This allows the user to overwrite a previous answer simply by clicking a new option.
    // This is the key change to allow re-selection.
    setUserAnswers(prev => ({ ...prev, [questionId]: answer }));
  };

  const handleSkip = (questionId) => {
    setUserAnswers(prev => ({ ...prev, [questionId]: null })); // Mark as skipped
  };
  ...
  const finishQuiz = () => {
    // Ensure all questions are marked (answered or explicitly skipped) before finishing
    questions.forEach(q => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6 p-4 bg-white rounded-xl shadow-lg border border-gray-100">
          ...
          <div className="flex flex-wrap gap-3">
            {questions.map((q, index) => {
              const statusClass = userAnswers[q.id] !== undefined
                ? (userAnswers[q.id] === null ? 'bg-red-200 text-red-800' : 'bg-green-200 text-green-800')
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${statusClass}
                  `}
                  title={userAnswers[q.id] === null ? 'Skipped' : userAnswers[q.id] !== undefined ? 'Answered' : 'Unanswered'}
                >
                  {index + 1}
                </button>
              );
        ...
        {/* Hidden section to ensure finish button is always visible if all questions are completed */}
